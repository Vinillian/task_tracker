...
class _TaskTrackerScreenState extends State<TaskTrackerScreen>
  ...
  void initState() {
    ...
    _loadData().then((_) {
      // Если после загрузки всё равно нет пользователя - создаем
      if (currentUser == null) {
        setState(() {
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      ...
      if (users.isNotEmpty) {
        ...
        // Создаем пользователя по умолчанию если нет данных
  ...
  void _addUser() {
    if (_userController.text.isNotEmpty) {
      setState(() {
        ...
        users.add(newUser);
        currentUser = newUser; // ← ВАЖНО: устанавливаем текущим пользователем
  ...
  void _addProject() {
    print('_addProject called'); // ← ДОБАВЬТЕ ЭТО
    if (currentUser == null) {
